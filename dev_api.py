import json
import socket
import requests, urllib3, os,sys, math
...
licensing_folder = os.path.join(workingdir, 'licensing')
                ...
                try:
                    ...
                    for imdata in __response_fabric__.json()['imdata']:
                         ...
                         if "controller" not in imdata['fabricNode']['attributes']['role'] and "spine" not in imdata['fabricNode']['attributes']['role']:
                            ...
                            elif "NLAM" in imdata['fabricNode']['attributes']['name'] and re.search(r'L10[1-5]', imdata['fabricNode']['attributes']['name']):
...
class license_functions():
    def __init__(self):
        ...
        self.license_info = {
            ...
            'counter': 1,            
            'title': 'Cisco Smart Licensing Usage',
            'title2': '',
            'title3': '',
            'title4': ''
    }
    ...
    def license_update(self):
        #Cisco Smart Licensing Support Info
        path = (r'C:\Users\equiros\Documents\Capacity Automation\Capacity App\Capacity_App\licensing\License_Report.xls')
        excel_data = pandas.read_excel(path)
        json_str = excel_data.to_json(orient='records')
        json_py = json.loads(json_str)
...
path = r'C:\Users\equiros\Documents\Capacity Automation\Capacity App\Capacity_App\templates\\'
...
## FXOS Display sites
def display_fxos():
    aci.post("/fxos_update")
    with open(path + 'fxos_base.html') as html:
        html_file = html.read()
    rendered_html = env.from_string(html_file)
    result = rendered_html.render(fxos=fxos_fun.fxo_status(),info=fxos_fun.fxos_info)
    print (result)
...
def display_aci():
    aci.post("/aci_update")
    with open(path + 'aci_base.html') as html:
        html_file = html.read()
    rendered_html = env.from_string(html_file)
    result = rendered_html.render(aci=aci_fun.aci_status(),info=aci_fun.aci_info)
    #print (result)

# WAN Update API
@api.route("/wan_update")
class wan(Resource):
    @api.doc('Get WAN Information')
    def post(self):
...
def display_wan():
    wan.post("/wan_update")
    with open(path + 'wan_base.html') as html:
        html_file = html.read()
    rendered_html = env.from_string(html_file)
    result = rendered_html.render(wan=wan_fun.wan_status(),info=wan_fun.wan_info)
    print (result)

#License Update API
@api.route("/license_update")
class license(Resource):
    @api.doc('Get License Information')
    def post(self):
        text = license_fun.license_update()
        return ("{0} {1} Successful --- {2}".format("license_update", text, license_fun.license_status()))

#License Display sites
def display_license():
    license.post("/license_update")
    with open(path + 'license_base.html') as html:
        html_file = html.read()
    rendered_html = env.from_string(html_file)
    result = rendered_html.render(license=license_fun.license_status(),info=license_fun.license_info)
    print(result)
